...
int train()
{
	ifstream input("/sdcard/words_eng.txt");
	string train_string;
    if(!input)
        return 1;
...
static jobjectArray make_row(JNIEnv *env, jsize count, char elements[][7])
{
...
jobjectArray Java_com_example_sidsharma_spellathon_MainActivity_spellList(JNIEnv *env, jobject obj, jstring jumbled)
{
    ...
    jobjectArray ret;
    const char *input;
    input = env->GetStringUTFChars(jumbled, NULL);
    char str[7];
    int i=0;
    do{
    ...
    if(solution_space.size()==0)
    	...
    	for(it = solution_space.begin(); it!= solution_space.end(); it++)
    	{
    	    ...
    	    string s = *it;
            do{
                data[j][i] = tolower(s[i]);
            }while(str[i++]!='\0');
            j++;
    	}

    jobjectArray jArr = make_row(env, solution_space.size(), data);
